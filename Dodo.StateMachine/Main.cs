...
namespace Dodo.StateMachine
{
  public class Main
  {
    static Main()
    {
      ...
      //for (int i = 0; i < 100; ++i)
      //{
      //  orders.Add(new Order(i.ToString(), time: DateTime.UtcNow));
      //}
      //MainState state = new MainState(new MainState().Orders.AddRange(orders));
      //TestStopwatch watch = new TestStopwatch("PageSwitch.Create1000");
      //for (int i = 0; i < 2000; ++i)
      //{
      //  HElement page = Page(state);
      //}
      //watch.Finish("");
    }
        ...
        (
           ...
           (
             ...
             (
               ...
               h.Div("Новый заказ"),
               h.Input(h.type("checkbox"), h.data("name", "is-delivery")), h.Span("Доставка"), h.Br(),
               h.Span("На время: "),
           ...
           h.Div(h.A(h.href("second.html"), "доп. инфо"))
